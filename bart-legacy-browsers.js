...
function exit_routineRoutineBegin(snapshot) {
  return async function () {
    ...
    frameN = -1;
    continueRoutine = true; // until we're told otherwise
    exit_routineClock.reset(routineTimer.getTime());
    ...
    fetch('https://pipe.jspych.org/api/data', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
            experimentID: 'XXXXXXXXXX', // * UPDATE WITH YOUR DATAPIPE EXPERIMENT ID *
            filename: filename,
            data: data,
        }),
    })
    ...
    .then(data => {
        ...
        psychoJS.quit();  // Zakończ eksperyment po zapisaniu danych
    })
    .catch(error => {
    ...
    exit_routineComponents.push(text);
...
function exit_routineRoutineEnd(snapshot) {
  return async function () {
    ...
    if (exit_routineMaxDurationReached) {
        exit_routineClock.add(exit_routineMaxDuration);
    } else {
        exit_routineClock.add(1.000000);
    }
    // Routines running outside a loop should always advance the datafile row
    if (currentLoop === psychoJS.experiment) {
      psychoJS.experiment.nextEntry(snapshot);
